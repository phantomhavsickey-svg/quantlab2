...
- **早停指标 = 验证集按日截面 Spearman Rank IC**（比 MSE 更贴合选股目标），patience 10
...
4. **Walk-Forward 严格性**：折 k 训练样本日期全部 < 该折 OOS 窗口起点；折内早停验证集 = 训练尾部 3 个月（同样早于 OOS 窗口）
...
- 训练结束打印每折 `best_rank_ic`（早停验证）与 `oos_rank_ic`（样本外）
- **平均 OOS RankIC > 0.02 可用，> 0.05 优秀，< 0 无效**（与 quantlab 口径一致，可直接对比 LightGBM）
- 回测报告输出年化收益/夏普/最大回撤/超额收益/信息比率，与 quantlab 在相同时间窗对比
- 断点续训：训练中断后重跑 `python main.py train` 会自动复用已有
  checkpoint 与已落盘的逐折 OOS 预测，不重复计算
...
注意：验证集 Rank IC（约 0.9）因 20 日前向标签的高度重叠被显著放大，
只有 OOS 窗口 IC 与回测收益是有意义的评估口径。
