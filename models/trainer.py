...
def _group_bounds(date_ints: np.ndarray) -> list[tuple[int, int]]:
    ...
    Returns:
        (order, starts, ends):order 为按日期稳定排序的下标,starts/ends
        给出排序后每个日期组的 [start, end) 区间(调用方需自行用 order
        重排 pred/y)
    """
...
def _split_fingerprint(store: "SequenceStore", wf_cfg: dict) -> dict:
    """决定折切分的参数集合 —— 变了则旧缓存不可复用。"""
    return {
        "horizon": int(store.horizon),
        "seq_len": int(store.seq_len),
        "val_months": int(wf_cfg.get("val_months", 3)),
        "embargo_days": int(wf_cfg.get("embargo_days", 0)),
        "min_train_months": int(wf_cfg["min_train_months"]),
        "retrain_months": int(wf_cfg["retrain_months"]),
        "min_oos_days": int(wf_cfg.get("min_oos_days", 20)),
    }


def _cache_meta_path(pred_path: str) -> str:
    return pred_path + ".meta.json"


def _cache_matches_split(pred_path: str, fp: dict) -> bool:
    """侧车文件缺失或口径不一致 → False(调用方应忽略缓存重训)。"""
    import json

    meta = _cache_meta_path(pred_path)
...
def _write_cache_meta(pred_path: str, fp: dict):
    import json

    with open(_cache_meta_path(pred_path), "w", encoding="utf-8") as f:
        json.dump(fp, f, ensure_ascii=False, sort_keys=True, indent=2)
...
class TransformerTrainer:
    ...
    def _fold_split(self, store: SequenceStore, all_idx: list,
        ...
        """折内时间切分:train / valid / OOS 三段互不重叠且标签不越界。
            ...
            train  [起点,       val_start)
            valid  [val_start,  purge_start)
            OOS    [ws,         we)

        purge_start = ws 往前 (horizon + embargo_days) 个交易日。前向 horizon
        日标签让 t 日样本的标签窗口覆盖 [t, t+horizon],不 purge 的话训练/验证
        样本的标签会伸进 OOS 窗口,早停指标也随之被污染。

        Returns:
            (train_mask, valid_mask, info)  info 含 purge_days / n_oos_days 等
        """
        wf = self.cfg["model"]["walk_forward"]
        purge = store.horizon + int(wf.get("embargo_days", 0))
        ws = np.datetime64(w.ws)
        ws_i = int(np.searchsorted(store.global_dates, ws))
        cut_i = max(ws_i - purge, 0)
        purge_start = store.global_dates[cut_i]
        val_start = np.datetime64(
            w.ws - pd.DateOffset(months=int(wf.get("val_months", 3))))

        valid_mask = (all_dates >= val_start) & (all_dates < purge_start)
        train_mask = all_dates < val_start

        oos_dates = np.unique(all_dates[(all_dates >= ws)
                                        & (all_dates < np.datetime64(w.we))])
        info = {
            "purge_days": ws_i - cut_i,
            "val_start": pd.Timestamp(val_start),
            "purge_start": pd.Timestamp(purge_start),
            "n_oos_days": int(len(oos_dates)),
        }
        if not valid_mask.any():
            raise RuntimeError(
                f"Fold {w.fold}: 验证段为空 —— val_start="
                f"{info['val_start']:%Y-%m-%d} 未在 purge_start="
                f"{info['purge_start']:%Y-%m-%d} 之前,需增大 "
                f"walk_forward.val_months({purge} 交易日 ≈ "
                f"{purge / 21:.1f} 个月)")
    ...
    def _run_fold(self, store: SequenceStore, all_idx: list,
        ...
        fold_metrics = {
            ...
            "valid_start": pd.Timestamp(all_dates[valid_mask].min()),
            "valid_end": pd.Timestamp(all_dates[valid_mask].max()),
    ...
    def walk_forward_train(self, store: SequenceStore, cfg: dict,
        ...
        每折(_fold_split):
            train = 日期 < val_start 的样本(滚动扩展)
            valid = [val_start, purge_start) —— 紧邻 purge 缺口之前,
                    与 train 无交集,用于早停/选权重
            OOS   = [ws, we),用 best checkpoint 预测
            purge_start = ws 往前 (horizon + embargo_days) 个交易日,
                    保证任何训练/验证样本的 20 日标签窗口都碰不到 OOS

        断点续训(resume=True):
            - 先比对 predictions.parquet.meta.json 的切分口径;不一致或
              缺侧车文件 → 整个缓存作废(否则新旧口径的折会混进同一个均值)
            - 该折 OOS 窗口已有预测 → 跳过
            - 该折 checkpoint 已存在但预测缺失 → 复用权重只做 OOS 预测
            - 每折完成后增量落盘 predictions.parquet + 侧车(中断不丢结果)
        ...
        min_oos_days = int(wf_cfg.get("min_oos_days", 20))
        split_fp = _split_fingerprint(store, wf_cfg)
        ...
        if resume and os.path.exists(pred_path):
            ...
            if not _cache_matches_split(pred_path, split_fp):
                logger.warning("已有 OOS 预测的切分口径与当前配置不一致"
                               "(purge/embargo/val_months 改过或无侧车文件)"
                               " → 忽略缓存,全部折重训")
                existing = None
        ...
        for w in windows:
            ...
            if oos_df is not None:
                ...
                _write_cache_meta(pred_path, split_fp)
                existing = save_df
        ...
        from utils.ic_stats import summarize_ic

        if len(oos_df) == 0:
            logger.warning("没有任何 OOS 预测,跳过 IC 汇总")
            return folds, oos_df

        di = oos_df["date"].values.astype("datetime64[D]").astype(np.int64)
        pooled = compute_rank_ic(oos_df["prediction"].to_numpy(),
                                 oos_df["forward_return"].to_numpy(), di)
        daily_ic = pooled["ic_by_date"]
        s = summarize_ic(daily_ic, horizon=store.horizon)
        fold_mean = np.mean([f["oos_rank_ic"] for f in folds
                             if np.isfinite(f["oos_rank_ic"])])
        logger.info(
            f"Walk-Forward 全部完成: OOS RankIC = {s['mean_ic']:.4f}"
            f" (按日合并 {s['n_days']} 天, NW({s['nw_lag']}) t={s['nw_t']:.2f},"
            f" p={s['nw_p']:.4f}, N_eff={s['n_eff']:.0f})"
            f" | 按折均值(仅供对照)={fold_mean:.4f}"
            f" (>0.02 可用, >0.05 优秀, <0 无效)")

        # 逐日 IC 落盘:只有几十 KB,却是所有显著性结论的唯一凭据
        ic_path = wf_cfg.get("daily_ic_csv", "reports/daily_ic.csv")
        try:
            os.makedirs(os.path.dirname(ic_path) or ".", exist_ok=True)
            daily_ic.rename("rank_ic").to_frame().to_csv(ic_path)
            logger.info(f"逐日 OOS Rank IC → {ic_path}"
                        f" ({len(daily_ic)} 行)")
        except OSError as e:
            logger.warning(f"逐日 IC 落盘失败: {e}")
        return folds, oos_df
